        ...
        if len(errors) > 0:
            ...
            with app.app_context():
                ...
                if(cognito_registration_process['success'] is not True):
                    return render_template(
                        "register.html",
                        errors=[cognito_registration_process['message']],
                        name=request.form["name"],
                        email=request.form["email"],
                        password=request.form["password"],
                    )
                user = Users(cognito_id=cognito_registration_process['data']['UserSub'])
                db.session.add(user)
                db.session.commit()
                db.session.flush()
                db.session.close()
...
@auth.route("/registration_confirm", methods=["POST", "GET"])
@check_registration_visibility
@ratelimit(method="POST", limit=10, interval=5)
def registration_confirm():
        if request.method == "POST":
            ...
            result = cognito_confirm_registration(username, code)
            if(result['success'] is not True):
                return render_template("registration_confirm.html", errors=[result['message']])
            session['info_message'] = "Registration confirmed successfully! You can now login."
            return redirect(url_for('auth.login'))
        else:
            infos = get_infos()
...
def login():
    ...
    if request.method == "POST":
        ...
        if(not authenticationResult['success']):
            errors.append(authenticationResult['error'])
            db.session.close()
        ...
        db.session.close()
        infos = get_infos()
        infos.append(session.pop('info_message', None))  # Retrieve and remove the message from session
        return render_template("login.html", errors=errors, infos=infos)
