...
def init_template_globals(app):
    ...
    app.jinja_env.globals.update(show_terminal=isUserInStage)
    # Add in JinjaEnums
...
def init_logs(app):
    LOG_FORMAT = (
        "%(asctime)s|CTFd|%(levelname)s|%(message)s|%(event)s|extra_data=%(extra_data)s|trace_id=%(trace_id)s"
    )
    DATE_FORMAT = "%Y-%m-%dT%H:%M:%S.%f"
    formatter = logging.Formatter(LOG_FORMAT, datefmt=DATE_FORMAT)
    formatter.converter = time.gmtime  # Use UTC for timestamp
